...
module.exports = class extends Command {
	...
	async getTypeOutput(msg, options) {
		...
		do {
			_choice = await msg
				.prompt(`Choose one of the following options: ${_options.join(', ')}`)
				.catch(() => ({ content: 'none' }));
		} while (
			![
				'file',
	...
	async eval(msg, code) {
		...
		if (typeof result !== 'string') {
			result =
        result instanceof Error ? result.stack : inspect(result, { depth: msg.flags.depth ? parseInt(msg.flags.depth) || 0 : 0, showHidden: Boolean(msg.flags.showHidden),
        });
		}
		return {
			success,
