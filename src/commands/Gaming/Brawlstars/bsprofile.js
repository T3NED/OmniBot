...
module.exports = class extends Command {
	...
	async run(msg, [resp]) {
		if(!resp) {
			if(msg.author.settings.ign.brawlstars) {
				try {
					const tag = msg.author.settings.ign.brawlstars;
					const player = await this.client.brawl.getPlayer(tag);
			...
			const player = await this.get_player(tag);
			switch (player) {
			case 'Invalid Tag':
	...
	async resolveResponse(response, orgMsg) {
		if(orgMsg.mentions.members.first()) {
			const tag = orgMsg.mentions.members.first().user.settings.ign.brawlstars;
			return tag;
		}
		else if(orgMsg.guild.members.get(response)) {
			const tag = orgMsg.guild.members.get(response).user.settings.ign.brawlstars;
			return tag;
		}
		else if(orgMsg.guild.members.find(m => m.user.username === response)) {
			const tag = orgMsg.guild.members.find(m => m.user.username === response).user.settings.ign.brawlstars;
			return tag;
		}
		else {
