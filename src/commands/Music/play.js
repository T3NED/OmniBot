...
module.exports = class extends Command {
	...
	async run(msg, [song]) {
		...
		const valid = await ytdl.validateURL(song);
		if (!valid) {
			search(song, async (err, res) => {
				...
				const ms = info.length_seconds * 1000;
	...
	async play(msg, data) {
		...
		data.dispatcher.on('error', async (err) => {
			...
			const { channel } = msg.guild.me.voice;
		...
		data.dispatcher.on('finish', async () => {
			const fetched = this.client.music.get(data.dispatcher.guildId);
			if (!fetched) return;
